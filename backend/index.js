...
app.post('/confirmation', async (req, res) => {
  ...
  // Insert the reset data into the MySQL database
  try {
    const resetData = await reset.create({ email, newPassword, otp });
    ...
    res.status(200).json({ success: true, message, resetData });
  } catch (error) {
...
app.post('/getcustomers', async (req, res) => {
  ...
  await  con.query(sql, [email], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).send('Error retrieving orders data');
    } else {
